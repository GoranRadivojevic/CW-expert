...
namespace CWExpert
{
    ...
    public class RingBufferChar
    {
        #region variable

        private char[] buf;     //  buffer for data
        public int wptr;	    //  write pointer
        public int rptr;	    //  read pointer
        private int size;	    //  buffer size
        private int mask;	    //  mask
        ...
        public RingBufferChar(int Size)
        {
            buf = new char[Size];
            mask = Size - 1;
            wptr = rptr = 0;
        ...
        public int Read(ref char[] dest, int cnt)
        {
            int free_cnt = ReadSpace();
            if (free_cnt == 0) return 0;
        ...
        public int Write(char[] src, int cnt)
        {
            int free_cnt = WriteSpace();
